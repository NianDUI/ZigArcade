...
MVP（FC）包含 iNES 1.0、Mapper 0/NROM、2A03 CPU、dot 时钟驱动的 PPU 寄存器/NMI/滚动地址时序、逐总线周期 OAM DMA、两个手柄端口和 `256×240` RGB 输出。当前实现还覆盖 MMC1、UNROM、CNROM 和 AOROM 的基础 bank 行为；PPU 已实现 `v/t/x/w`、奇帧 dot skip、渲染期 `v/t` 推进、可见区寄存器写入回放，以及背景逐 dot 取数/16 位 shift-register 与精灵 secondary OAM/257–320 dot pattern 取数的直接像素输出。sprite-0 hit 与 sprite overflow（含八精灵后的错误对角字节搜索）已通过对应公开 ROM 组；精确 clear/copy micro-op 边界仍待验证。APU 已有 Pulse 1/2、Triangle、Noise 与 DMC 的基础时钟和 PCM 链，macOS 在显式 `--audio` 下可输出到 CoreAudio AudioUnit（AudioQueue 为试验后端）。仍不含持久存档、录像或联网。
...
| P4 | Mapper 1/2/3/4、APU、可选宿主音频 | 每个 mapper 有 bank/mirroring 单测和公开 ROM 回归；现已覆盖 Mapper 1/2/3/4/7 与多通道 APU 基础链，MMC3 的背景/精灵 pattern A12 IRQ 已接入，公开 PPU/Mapper 与音频精度基线仍待完成 |
...
PPU 为 341 dot × 262 scanline，约 60.0988 FPS。`tickDot()` 是唯一驱动寄存器、NMI、滚动地址、背景/精灵像素、sprite overflow 和取数/IRQ 可观察状态的规范实现。CPU bus 访问须有绝对 CPU/PPU tick；已实现 pre-render odd-frame dot skip、PPU open bus、VBlank 起始前的 `$2002` 读取抑制、渲染期 coarse X/fine Y 推进、dot 257 水平拷贝、pre-render 280–304 垂直拷贝，以及背景名称表/属性/pattern 在 1/3/5/7 dot 的取数、每 8 dot 的 16 位 pattern/attribute shift-register 装载和直接像素输出（含 321–336 dot prefetch）。精灵在 dot 1–64 清 secondary OAM、dot 65–256 为下一行选择至多 8 项并维护溢出错误路径、dot 257–320 进行 dummy/低 plane/高 plane 取数，将锁存 pattern/属性/X 坐标用于下一行像素合成；背景和精灵 pattern 地址均会逐 dot 送入 MMC3，A12 低电平持续至少 8 dot 后的上升沿可重载/递减 IRQ 计数器并在下一 CPU 指令边界交付 IRQ。CPU `$2007` 访问和展示快照不会参与该时钟。Y=`$FF` 保持 off-screen。VBlank/NMI、palette/VRAM read buffer、OAM 访问、CPU RMW 双写、sprite-0 hit 与 sprite overflow 已有公开 ROM 通过证据；PPU clear/copy micro-op 仍待更细测试。

已实现：`$2000-$2007` 的 `v/t/x/w` 锁存、`$2002` 清 VBlank/重置写锁存、pattern/nametable/palette 镜像、VBlank NMI、背景 tile/attribute、背景逐 dot fetch/shift-register/像素输出、secondary OAM、逐 dot 精灵 pattern 取数、精灵透明/优先级/基础 8 精灵限制、sprite-0 hit，以及八精灵后的错误字节循环 sprite overflow 搜索。明确 `$3000-$3EFF` nametable mirror 与 `$3F10/$14/$18/$1C` palette mirror。内部先输出调色板索引，再映射 64 色 RGB。待完成：非渲染期总线和精确 PPU clear/copy micro-op 时序校正。

`Cartridge` 仅解析校验 iNES；Mapper 提供 `cpuRead/cpuWrite/ppuRead/ppuWrite/mirroring`。当前支持 Mapper 0：16 KiB PRG-ROM `$8000-$FFFF` 镜像或 32 KiB 直映射、CHR-ROM 写忽略或 CHR size=0 时的 8 KiB CHR-RAM；Mapper 1/MMC1：5-bit 串行寄存器、16/32 KiB PRG 与 4/8 KiB CHR bank、单屏/垂直/水平 mirroring，支持 CHR-ROM 或 8 KiB CHR-RAM；标准 iNES MMC1 范围限定为 16–256 KiB PRG 和至多 128 KiB CHR，单个 16 KiB PRG bank 在所有模式下镜像，外部 bank 的板型变种暂不接受；Mapper 2/UNROM：可切换 `$8000-$BFFF` 的 16 KiB PRG bank、固定 `$C000-$FFFF` 的末 bank 和 8 KiB CHR-RAM；Mapper 3/CNROM：16/32 KiB 固定 PRG 映射、CPU 写 `$8000-$FFFF` 选择一个 8 KiB CHR-ROM bank，单 bank 版型的选择写自然保持 bank 0，CHR-ROM 不可写；Mapper 7/AOROM：32–256 KiB PRG，CPU 写 `$8000-$FFFF` 选择一个 32 KiB PRG bank（低三位）及单屏名称表（bit 4），仅支持 8 KiB CHR-RAM。Mapper 0/1/2/3 有易失 8 KiB PRG-RAM `$6000-$7FFF`，MMC1 以 PRG bank bit 4 控制这段 RAM 的读写；标准 AOROM 不暴露 PRG-RAM。battery 标志仍拒绝，不创建持久存档。初版拒绝 NES 2.0、trainer、four-screen 和其他未知 mapper。
