...
MVP（FC）包含 iNES 1.0、Mapper 0/NROM、2A03 CPU、dot 时钟驱动的 PPU 寄存器/NMI/滚动地址时序、逐总线周期 OAM DMA、两个手柄端口和 `256×240` RGB 输出。当前实现还覆盖 MMC1、UNROM、CNROM 和 AOROM 的基础 bank 行为；PPU 已实现 `v/t/x/w`、奇帧 dot skip、渲染期 `v/t` 推进、可见区寄存器写入回放，以及背景逐 dot 取数/16 位 shift-register 与精灵 secondary OAM/257–320 dot pattern 取数的直接像素输出。sprite overflow 已按 dot 扫描并覆盖八精灵后的错误对角字节搜索；精确 clear/copy micro-op 边界仍待公开测试 ROM 验证。APU 已有 Pulse 1/2、Triangle、Noise 与 DMC 的基础时钟和 PCM 链，macOS 在显式 `--audio` 下可输出到 CoreAudio AudioUnit（AudioQueue 为试验后端）。仍不含持久存档、录像或联网。
