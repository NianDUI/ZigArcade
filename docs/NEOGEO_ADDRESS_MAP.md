...
| `$3A0001` 等 | system-control latch | palette bank、vector source、save/memory-card 控制等 | `system_control.zig` 已记录 `$3A0003/$3A0013` 的 BIOS/卡带 vector source 与 `$3A000F/$3A001F` 的 palette bank 0/1；写数据被忽略，只有 odd byte lane 有效；vector source 已连接 `$000000-$00007F`，palette bank 尚未连接 palette RAM |
