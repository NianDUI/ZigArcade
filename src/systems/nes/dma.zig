...
pub const OamDma = struct {
    ...
    /// CPU write to $4014. This defines the local parity convention used by
    /// the integrated machine and is asserted by the parity tests below.
...
test "OAM DMA performs 513 cycles from an even start and reads mapper source" {
    ...
    try std.testing.expectEqual(@as(u16, 0x8000), bus.accesses()[1].address);
}

test "OAM DMA performs an extra alignment cycle from an odd start" {
    const Mapper0 = @import("mapper0.zig").Mapper0;
    var prg: [16 * 1024]u8 = [_]u8{0} ** (16 * 1024);
    var mapper = Mapper0{
        .prg_rom = &prg,
