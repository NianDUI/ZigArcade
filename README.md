...
`nes` 当前接受 iNES 1.0 的 Mapper 0/NROM（16/32 KiB PRG、8 KiB CHR-ROM 或 CHR-RAM）、Mapper 1/MMC1（至少 32 KiB PRG、CHR-ROM 或 CHR-RAM）、Mapper 2/UNROM（CHR-RAM、至少两个 16 KiB PRG bank）、Mapper 3/CNROM（16/32 KiB 固定 PRG、至少两个 8 KiB CHR-ROM bank）和 Mapper 7/AOROM（32–256 KiB PRG、CHR-RAM）。MMC1 支持串行 PRG/CHR bank 切换及单屏、水平、垂直镜像，CNROM 的 `$8000-$FFFF` 写入选择 CHR bank，AOROM 的 `$8000-$FFFF` 写入选择 32 KiB PRG bank 与单屏名称表；Mapper 0/1/2/3 提供 8 KiB 易失 PRG-RAM，AOROM 标准版型不提供 PRG-RAM。默认 `--renderer auto` 会在 TTY 中优先探测 Ghostty 的 Kitty 图像协议，失败时回退 ANSI；强制 `--renderer kitty` 在探测失败时返回错误。电池存档仍明确不支持。`WASD` 或方向键映射方向、`Z/X` 映射 A/B、Enter=Start、Tab=Select、Esc 退出；当前输入以单帧采样处理按键，持续按键/可靠 key-up 会在接入 Kitty keyboard protocol 后完善。
