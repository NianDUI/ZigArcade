...
对合法持有的 SMB ROM，可将一次交互日志作为输入回放，执行 `scripts/smb-regression.zsh <rom.nes> <输入日志>`。它在伪终端中重放输入，并校验音频非静音、输出设备运行及无 render error；生成的运行日志保存在 `/tmp/zigarcade-smb-regression.*`。
